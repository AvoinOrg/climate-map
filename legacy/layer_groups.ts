...
import { assert, execWithMapLoaded, originalLayerDefs } from '../src/map/utils'
import * as Analytics from '../src/map/analytics'

const backgroundLayerGroups = { terramonitor: true }
interface ILayerGroupState {
  [s: string]: boolean
}
...
export const hideAllLayersMatchingFilter: (filterFn: (group: string) => boolean) => void = (filterFn) => {
