...
const enableDefaultLayers = () => {
  ...
  for (const group of enabledGroups) {
    const normalLayers = layerGroupDefinitions[group].filter((l: any) => typeof l === 'string') as string[]
...
onMapLoad(() => {
  ...
  const hashParams: IHashParams = window.location.hash
    .replace(/^[#?]*/, '')
    .split('&')
    .reduce((prev, item) => Object.assign({ [item.split('=')[0]]: item.split('=')[1] }, prev), {}) as IHashParams
  ...
  try {
    ...
    for (const layer of layers) {
      LayerGroupState.enableGroup(layer)
    }
  } catch (e) {}
}) // /map onload
