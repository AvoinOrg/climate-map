...
const colormapToStepExpr = (colormap: number[][], minValue: number, maxValue: number, expr: Expression) => {
  ...
  const cmap = colormap.map((x) => x.map((c) => Math.round(255 * c))).map(cmapToRGBA as any)
...
export const executeDelayedMapOperations = () => {
  for (const op of delayedMapOperations) {
    op()
  }
...
export const execWithMapLoaded = (fn: () => void) => {
  if (isMapLoaded()) {
    ...
    delayedMapOperations.push(fn)
  }
}

interface ISourceDefs {
  [s: string]: mapboxgl.AnySourceData
}
export const originalSourceDefs: ISourceDefs = {}
interface ILayerDefs {
  [s: string]: Layer
}
