...
const mbSourceToSource: (source: mapboxgl.AnySourceData) => SourceType | null = (source: mapboxgl.AnySourceData) => {
  switch (source.type) {
    case 'geojson':
      ...
      const data = typeof source.data === 'string' ? `{"type": "FeatureCollection","features":[]}` : source.data
...
export const panTo = (lon: number, lat: number) => new View({ center: fromLonLat([lon, lat]) }).animate()

export const flyTo = (lon: number, lat: number, zoom: number) => new View({ center: fromLonLat([lon, lat]), zoom })
