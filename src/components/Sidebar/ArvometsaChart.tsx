...
import { roundToSignificantDigits, assert, pp, getGeoJsonGeometryBounds, execWithMapLoaded } from '../../map/utils';
...
const stepsToLinear = (min: number, max: number, steps: any[]) => {
...
const ChartComponent = props => {
    ...
    useEffect(() => {
        ...
        if (chartUpdateFunction) {
            chartUpdateFunction(chart)
        }
        setChart(chart)

        return chart2.destroy
    }, [canvasRef, chart, chartOptions, chartUpdateFunction])
...
// TODO: add a button to goto location, aka:
