...
function OmaihkaUI() {
  ...
  const swapLayer = () =>
    LayerGroupState.enableOnlyOneGroup(
      topsoilVisible ? "fi-omaihka-subsoil" : "fi-omaihka-topsoil"
    );
  ...
  if (hasFeature) {
    ...
    const soilFeatures = querySourceFeatures(sourceName, "default").filter(
      (x) => x.properties.lohko === props.lohko
    );

    const topsoilCodes = Array.from(
      new Set(soilFeatures.map((x) => x.properties.pintamaalaji_koodi))
    );
    const subsoilCodes = Array.from(
      new Set(soilFeatures.map((x) => x.properties.pohjamaalaji_koodi))
    );
    ...
    info = (
      <ul style={ulStyle}>
        ...
        <li>
          <strong>Lohkon numero</strong>: {props.__ext_id_orig}{" "}
          (peltolohkokannasta {props.lohko})
        </li>
        <li>
        ...
        <li>
          <strong>Ympärys</strong>:{" "}
          {props.ymparys ? `${pp(props.ymparys, 3)} m` : ""}
        </li>
        <li>
        ...
        <li>
          <strong>Nurmen perustamisvuosi</strong>:{" "}
          {props.nurmen_perustamisvuosi}
        </li>
        <li>
          <strong>Pellon ikä</strong>:{" "}
          {props.pelto_ika ? `${props.pelto_ika} vuotta` : ""}
        </li>
        <li>
  ...
  return (
    <div className="grid-parent grid-parent-report-closed">
      <Paper className="grid-col1" elevation={5}>
        <Container>
          <p>
            <a href="https://www.luke.fi/projektit/omaihka/">
              Omaihka -hankekartta 2020-2022
            </a>
          </p>
          <p>
            <strong>
              {topsoilVisible
                ? "Pintamaalajit näkyvillä"
                : "Pohjamaalajit näkyvillä"}
            </strong>
          </p>
          <p>
