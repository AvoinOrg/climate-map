...
import { Theme, TableContainer, Paper, Container, FormControl, InputLabel, OutlinedInput } from '@mui/material'
  ...
  useEffect(() => {
    ...
    fetchSource('valio-carbon-grass-fields/centroids.geojson').then((data) => {
      const features = data.features.sort((a, b) =>
        Number(a.properties['TILATUNNUS']) > Number(b.properties['TILATUNNUS']) ? 1 : -1
      )
      setData(features)
    })
  ...
  useEffect(() => {
    if (data) {
      if (searchValue === '') {
        ...
        setFilter('valio-carbon-grass-fields-fill', ['!in', 'TILATUNNUS', ...result])
        setFilter('valio-carbon-grass-fields-outline', ['!in', 'TILATUNNUS', ...result])
        setFilter('valio-carbon-grass-fields-label', ['!in', 'TILATUNNUS', ...result])
        setFilter('valio-carbon-grass-fields-pin', ['!in', 'TILATUNNUS', ...result])
      }
    }
    // eslint-disable-next-line
  }, [searchValue])
                ...
                {data
                  ...
                  .map((x) => (
                    <li key={x.properties['TILATUNNUS']}>
                      <Link to={`/layers/valio-carbon-grass-fields?field=${x.properties['TILATUNNUS']}`}>
