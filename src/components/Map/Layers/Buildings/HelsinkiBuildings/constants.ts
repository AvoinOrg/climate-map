import { ILayerOptions, LayerLevel } from './types'

export const layerOptions: ILayerOptions = {
  [LayerLevel.Helsinki_buildings]: { minzoom: 0, maxzoom: 30, serverId: 'helsinki-buildings', layerMinzoom: 0, layerMaxzoom: 30},
  [LayerLevel.Helsinki_apartment_buildings]: { minzoom: 0, maxzoom: 30, serverId: 'helsinki-apartment-buildings', layerMinzoom: 0, layerMaxzoom: 30 },
} as const

export const buildingHelBhsysClass: any = {
  1: 'District heating',
  2: 'Light fuel oil',
  3: 'Heavy fuel oil',
  4: 'Direct electric heating',
  9: 'Air-to-water heat pumpu, Ground source heat pump etc',
}

export const buildingHePaybackClass: any = {
  1: 'District heat consumption',
  2: 'Electricity consumption',
  3: 'Year Cost',
  4: 'Mwh/a',
  5: '€/a',
  6: '€',
  7: 'snt/kWh',
  8: '€/MWh'
}

// Estimated annual energy consumption [kWh/m3,a]
export const energyConsumption: any = {
  consumption: [
    {
      year: '1975',
      Oil: '70.917',
      dis_heating: '66.272',
      direct_heating: '62.625',
      awhpump: '45.955',
      ghpump: '35.928',
      OilL: '64.477',
      dis_heatingL: '59.863',
      direct_heatingL: '0',
      awhpumpL: '39.505',
      ghpumpL: '29.632',
      OilS: '6.44',
      dis_heatingS: '6.409',
      direct_heatingS: '62.625',
      awhpumpS: '6.45',
      ghpumpS: '6.295',      
      estenergCons: '35.928', 
    },
    {
      year: '1976',
      Oil: '64.053',
      dis_heating: '59.901',
      direct_heating: '56.309',
      awhpump: '41.838',
      ghpump: '32.857',
      OilL: '57.639',
      dis_heatingL: '53.518',
      direct_heatingL: '0',
      awhpumpL: '35.423',
      ghpumpL: '26.566',
      OilS: '6.414',
      dis_heatingS: '6.383',
      direct_heatingS: '56.309',
      awhpumpS: '6.416',
      ghpumpS: '6.290',
      estenergCons: '32.857',
    },
    {
      year: '1978',
      Oil: '55.573',
      dis_heating: '52.022',
      direct_heating: '48.554',
      awhpump: '36.507',
      ghpump: '28.958',
      OilL: '49.188',
      dis_heatingL: '45.668',
      direct_heatingL: '0',
      awhpumpL: '30.103',
      ghpumpL: '22.673',
      OilS: '6.385',
      dis_heatingS: '6.354',
      direct_heatingS: '48.554',
      awhpumpS: '6.404',
      ghpumpS: '6.285',
      estenergCons: '28.958',
    },
    {
      year: '1985',
      Oil: '53.097',
      dis_heating: '49.738',
      direct_heating: '46.352',
      awhpump: '35.047',
      ghpump: '27.861',
      OilL: '46.72',
      dis_heatingL: '43.391',
      direct_heatingL: '0',
      awhpumpL: '28.657',
      ghpumpL: '21.578',
      OilS: '6.377',
      dis_heatingS: '6.347',
      direct_heatingS: '46.352',
      awhpumpS: '6.391',
      ghpumpS: '6.283',
      estenergCons: '27.861',
    },
    {
      year: '2003',
      Oil: '48.289',
      dis_heating: '45.339',
      direct_heating: '43.156',
      awhpump: '32.839',
      ghpump: '26.574',
      OilL: '40.638',
      dis_heatingL: '37.719',
      direct_heatingL: '0',
      awhpumpL: '25.158',
      ghpumpL: '19.004',
      OilS: '7.651',
      dis_heatingS: '7.62',
      direct_heatingS: '43.156',
      awhpumpS: '7.68',
      ghpumpS: '7.57',
      estenergCons: '26.574',
    },
    {
      year: '2008',
      Oil: '47.917',
      dis_heating: '44.999',
      direct_heating: '42.832',
      awhpump: '32.612',
      ghpump: '26.411',
      OilL: '40.269',
      dis_heatingL: '37.38',
      direct_heatingL: '0',
      awhpumpL: '24.933',
      ghpumpL: '18.841',
      OilS: '7.65',
      dis_heatingS: '7.619',
      direct_heatingS: '42.832',
      awhpumpS: '7.679',
      ghpumpS: '7.57',
      estenergCons: '26.411',
    },
    {
      year: '2010',
      Oil: '40.39',
      dis_heating: '38.03',
      direct_heating: '36.216',
      awhpump: '28.204',
      ghpump: '23.408',
      OilL: '32.764',
      dis_heatingL: '30.435',
      direct_heatingL: '0',
      awhpumpL: '20.554',
      ghpumpL: '15.848',
      OilS: '7.626',
      dis_heatingS: '7.595',
      direct_heatingS: '36.216',
      awhpumpS: '7.65',
      ghpumpS: '7.56 ',
      estenergCons: '23.408',
    },
    {
      year: '2012',
      Oil: '37.074',
      dis_heating: '34.92',
      direct_heating: '33.122',
      awhpump: '26.018',
      ghpump: '21.708',
      OilL: '29.844',
      dis_heatingL: '27.72',
      direct_heatingL: '0',
      awhpumpL: '18.776',
      ghpumpL: '14.539',
      OilS: '7.23',
      dis_heatingS: '7.2',
      direct_heatingS: '33.122',
      awhpumpS: '26.018',
      ghpumpS: '21.708',
      estenergCons: '21.708',
    },
    {
      year: '2018',
      Oil: '34.911',
      dis_heating: '32.903',
      direct_heating: '31.101',
      awhpump: '24.632',
      ghpump: '20.722',
      OilL: '27.843',
      dis_heatingL: '25.865',
      direct_heatingL: '0',
      awhpumpL: '17.558',
      ghpumpL: '13.713',
      OilS: '7.068',
      dis_heatingS: '7.038',
      direct_heatingS: '31.101',
      awhpumpS: '7.074',
      ghpumpS: '7.009',
      estenergCons: '20.722',
    },
  ],
}
/*export const buildingHelEstimatedClass: any = {
    1: 'Estimated yearly heating related CO2-emissions: [tCO2/a]',
    2: 'Estimated yearly heating related CO2-emissions: [kgCO2/(m3,a)]',
    3: 'Estimated yearly heating related CO2-emissions: [kgCO2/kWh]',
    4: 'Energy consumption reduction potential by switching to GSHP: MWh/a',
    5: 'CO2-emission reduction potential by switching to GSHP: tCO2/a',
    6: 'Energy consumption reduction potential by switching to AWHP: MWh/a',
    7: 'CO2-emission reduction potential by switching to AWHP: tCO2/a',
  }
*/
