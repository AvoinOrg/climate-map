import 'ol/ol.css';
import { MapLayerMouseEvent, PopupOptions, Layer, AnySourceData, Style, Expression } from 'mapbox-gl';
import { sanitize } from 'dompurify';
import { originalSourceDefs, originalLayerDefs, invertLayerTextHalo, assert } from './utils';
...
const mbSourceToSource: (source: mapboxgl.AnySourceData) => SourceType | null = (source: mapboxgl.AnySourceData) => {
  switch (source.type) {
    case 'geojson':
    ...
    case 'vector':
      const opts: Options = {
        ...
        tileUrlFunction: (coord: number[]) => tileUrlFunction(source.tiles![0], coord),
      };
      if (source.attribution) { opts.attributions = source.attribution; }
      return new VectorTileSource(opts);
    case 'raster':
      assert(source.tiles!.length === 1, `Source URL: ${JSON.stringify(source)}`)
...
interface ISource { [id: string]: SourceType }
interface ILayer { [id: string]: LayerOL }
interface ISourceType { [id: string]: string }
interface IMapLayer { [id: string]: boolean }
const sources: ISource = {};
const layers: ILayer = {};
const sourceTypes: ISourceType = {}; // TODO: get source type somewhere else..
const mapLayers: IMapLayer = {};

interface IOLUIDToName { [id: string]: string }
const olUIDToLayerName: IOLUIDToName = {};

interface IMBSource { [id: string]: AnySourceData }
interface IMBLayer { [id: string]: Layer }
...
export const genericPopupHandler: (layerId: string | string[], fn: (e: MapLayerMouseEvent) => void) => void =
  (layerId, fn) => {
...
export const setLayoutProperty = (layerId: string, name: string, value: any) => {
  if (name === 'visibility') {
    const layer = mbLayers[layerId];
...
export const setPaintProperty = (layerId: string, name: string, value: Expression | string | number) => {
  const layer = mbLayers[layerId];
  // @ts-ignore
