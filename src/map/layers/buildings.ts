...
genericPopupHandler("fi-buildings-fill", (e) => {
  ...
  if (props.gid) {
    ...
    nls = `
        ...
        ${
          approxArea > 1 &&
          `<br/>Estimated floorage: ${pp(
            approxArea,
            2
          )} m<sup>2</sup> per floor`
        }
        ${
          approxArea > 1 &&
          approxVolume &&
          `<br/>Estimated volume: ${pp(
            approxVolume,
            2
          )} m<sup>3</sup> per floor`
        }
        `;
  }
  const html = vrk + nls;
...
addSource("helsinki-buildings", {
  type: "vector",
  tiles: [
    "https://server.avoin.org/data/map/helsinki-buildings/{z}/{x}/{y}.pbf",
  ],
  maxzoom: 14,
...
addSource("helsinki-puretut", {
  type: "vector",
  tiles: [
    "https://server.avoin.org/data/map/hel-puretut/{z}/{x}/{y}.pbf.gz?v=0",
  ],
  maxzoom: 14,
  // Bounds source: https://koordinates.com/layer/4257-finland-11000000-administrative-regions/
  // select ST_Extent(ST_Transform(ST_SetSRID(geom,3067), 4326))
...
genericPopupHandler("helsinki-puretut-fill", (e) => {
  ...
  e.features.forEach((f) => {
    ...
    const buildingIdText =
      p.vtj_prt && p.ratu ? `${p.vtj_prt} (${p.ratu})` : p.vtj_prt || p.ratu;
    const s = `
        <p>
      ...
      b.replace(
        ...
        buildingIdMap[b]
          ? buildingIdMap[b].reduce(
              (c: string, d: string) =>
                c ? `${c}, ${d}` : `<strong>Building ID:</strong> ${d}`,
              ""
            )
          : ""
      ),
...
registerGroup("fi-buildings", ["fi-buildings-fill", "fi-buildings-outline"]);

registerGroup("helsinki-buildings", [
  "helsinki-buildings-fill",
  "helsinki-buildings-outline",
  "helsinki-buildings-co2",
  "helsinki-puretut-fill",
  "helsinki-puretut-outline",
  "helsinki-puretut-sym",
]);
